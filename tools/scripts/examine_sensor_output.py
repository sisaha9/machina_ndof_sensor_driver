...
def main(bagfile, topic):
    ...
    for msg in tqdm(read_ros2_messages(source=str(bagfile), topics=[topic])):
        row_info = {}
        if prev_msg:
            row_info["publisher_time_diff_sec"] = get_difference_in_time(prev_msg.ros_msg.stamp, msg.ros_msg.stamp)
        else:
            prev_msg = msg
            continue
        for i in range(len(msg.ros_msg.sensors)):
            row_info[f"sensor_{i}_time_diff_sec"] = get_difference_in_time(prev_msg.ros_msg.sensors[i].header.stamp, msg.ros_msg.sensors[i].header.stamp)
            row_info[f"sensor_{i}_average_deviation"] = np.abs(np.array(prev_msg.ros_msg.sensors[i].data.data) - np.array(msg.ros_msg.sensors[i].data.data)).mean()
            row_info[f"sensor_{i}_data_valid"] = np.uint8(np.isclose(np.sum(msg.ros_msg.sensors[i].data.data) - msg.ros_msg.sensors[i].data.data[-1], msg.ros_msg.sensors[i].data.data[-1]))
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--bagfile", help="ROS2 bagfile to examine", type=Path, required=True)
